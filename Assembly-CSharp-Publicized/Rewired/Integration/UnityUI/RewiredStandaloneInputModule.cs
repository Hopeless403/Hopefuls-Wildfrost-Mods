...
namespace Rewired.Integration.UnityUI
{
  ...
  public sealed class RewiredStandaloneInputModule : RewiredPointerInputModule
  {
    ...
    public bool ShouldIgnoreEventsOnNoFocus() => !ReInput.isReady || ReInput.configuration.ignoreInputWhenAppNotInFocus;
    ...
    public bool GetButtonDown(Player player, int actionId) => actionId >= 0 && player.GetButtonDown(actionId);

    public bool GetNegativeButton(Player player, int actionId) => actionId >= 0 && player.GetNegativeButton(actionId);

    public bool GetNegativeButtonDown(Player player, int actionId) => actionId >= 0 && player.GetNegativeButtonDown(actionId);

    public float GetAxis(Player player, int actionId) => actionId < 0 ? 0.0f : player.GetAxis(actionId);
