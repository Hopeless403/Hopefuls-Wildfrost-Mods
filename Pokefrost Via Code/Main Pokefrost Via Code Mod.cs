...
[HarmonyPatch]
public partial class Pokefrost_Via_CodeMod : WildfrostMod
{
    // it should be named this to show up as a Singleton in UnityExplorer
    public static Pokefrost_Via_CodeMod instance;
    public Pokefrost_Via_CodeMod(string modDirectory) : base(modDirectory)
    {
        instance = this;
    }
    public override string GUID => "hope.wildfrost.pokefrostviacode";
    public override string[] Depends => new string[] { };
    public override string Title => "Pokefrost Via Code";
    ...
    [HarmonyPatch(typeof(WildfrostMod), nameof(WildfrostMod.ImagePath))]
    public static void Postfix(ref string __result, WildfrostMod __instance, string fileName)
    {
        if (__instance != instance)
        {
            __result = Path.Combine(__instance.ImagesDirectory, fileName);
            return;
        }

        var poke = Bootstrap.Mods.First(m => m.GUID == "websiteofsites.wildfrost.pokefrost");
        __result = Path.Combine(poke.ImagesDirectory, fileName);
    }

    //[HarmonyPatch(typeof(CardDataBuilder), nameof(CardDataBuilder.SetSprites), typeof(string), typeof(string))] [HarmonyPrefix]
    public static bool SetSprites(ref CardDataBuilder __result, CardDataBuilder __instance)
    {
        if (__instance.Mod != instance)
            return true;

        __instance.SetSprites(instance.IconSprite, instance.IconSprite);
        __result = __instance;
        return false;
    }
