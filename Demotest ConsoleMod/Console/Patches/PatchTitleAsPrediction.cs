        ...
        public static class PatchTitleAsPrediction
        {
            ...
            static IEnumerator Postfix(IEnumerator __result, object __instance)
            {
                ...
                if (command.id.Contains("spawn"))
                {
                    ...
                    var source = group.list.Cast<CardData>().Where(a => a.cardType.unit && (a.name.ToLower().Contains(args.ToLower()) || (a.titleKey != null && !a.titleKey.IsEmpty && a.title.ToLower().Contains(args.ToLower()))));
                    command.predictedArgs = source.Select(data => data.titleKey == null || data.titleKey.IsEmpty ? data.name : $"{data.name} // {data.title}").ToArray();
                }
                else if (command.id.Contains("card"))
                {
                    var group = AddressableLoader.groups["CardData"];
                    var source = group.list.Cast<CardData>().Where(a => a.name.ToLower().Contains(args.ToLower()) || (a.titleKey != null && !a.titleKey.IsEmpty && a.title.ToLower().Contains(args.ToLower())));
                    command.predictedArgs = source.Select(data => data.titleKey == null || data.titleKey.IsEmpty ? data.name : $"{data.name} // {data.title}").ToArray();
                }
                else if (command.id.Contains("upgrade"))
                {
                    var group = AddressableLoader.groups["CardUpgradeData"];
                    var source = group.list.Cast<CardUpgradeData>().Where(a => a.name.ToLower().Contains(args.ToLower()) || (a.titleKey != null && !a.titleKey.IsEmpty && a.title.ToLower().Contains(args.ToLower())));
                    command.predictedArgs = source.Select(data => data.titleKey == null || data.titleKey.IsEmpty ? data.name : $"{data.name} // {data.title}").ToArray();
                }
                yield return null;
            }
