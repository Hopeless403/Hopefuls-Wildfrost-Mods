                ...
                if (args.Length > 0)
                {
                    if (Console.hover != null)
                    {
                        ...
                        else if (Console.hover.enabled || (References.Player.entity.display is CharacterDisplay display && display.deckDisplay.gameObject.activeSelf))
                        {
                            ...
                            Entity applier = null;
                            yield return CardManager.CreateCard(AddressableLoader.groups["CardData"].lookup["Junk"] as CardData, null, References.Player, false, c => applier = c.entity);
                            Entity target = Console.hover;
                            string[] strArray = Split(args);
                            int count = 1;
                            ...
                            if (statusData != null)
                            {
                                yield return StatusEffectSystem.Apply(Console.hover, applier, statusData, count);
                                target.statusEffects = target.statusEffects.Where(s => !(s.count <= 0)).ToList();
                                target.display.promptUpdateDescription = true;
                                target.PromptUpdate();
                    ...
                    if (Console.hover != null)
                    {
                        ...
                        else if (Console.hover.enabled || (References.Player.entity.display is CharacterDisplay display && display.deckDisplay.gameObject.activeSelf))
                        {
                            ...
                            if (statusData != null)
                            {
                                ...
                                target.attackEffects = target.attackEffects.Where(s => !(s.count <= 0)).ToList();
                                target.display.promptUpdateDescription = true;
                                target.PromptUpdate();
