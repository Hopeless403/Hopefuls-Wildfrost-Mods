...
public class ConsoleArgsDisplay : MonoBehaviourRect
{
  ...
  public string TopCommand
  {
    get
    {
      return ((IEnumerable<Console.Command>) this.commands).Last<Console.Command>().id + (((IEnumerable<Console.Command>) this.commands).Last<Console.Command>().format != ((IEnumerable<Console.Command>) this.commands).Last<Console.Command>().id ? " " : "");
    }
  }
  ...
  private void Update()
  {
    this.rectTransform.anchoredPosition = Delta.Lerp(this.rectTransform.anchoredPosition, this.targetPos, 0.1f, Time.deltaTime);
  }
