...
public class Hit
{
  ...
  public Hit(Entity attacker, Entity target)
  {
    ...
    if ((bool) (Object) attacker)
    {
      ...
      this.countsAsHit = attacker.HasAttackIcon();
      if (this.countsAsHit)
  ...
  public void AddAttackerStatuses()
  {
    ...
    {
      int attackEffectAmount = this.CalculateAttackEffectAmount(attackEffect.data, attackEffect.count);
      if (attackEffectAmount > 0)
        this.AddStatusEffect(attackEffect.data, attackEffectAmount);
    }
  }

  private int CalculateAttackEffectAmount(StatusEffectData data, int statusEffectStacks)
  {
    if (this.attacker.silenced)
      return 0;
    return !data.stackable ? statusEffectStacks : Mathf.Max(0, Mathf.RoundToInt((float) (statusEffectStacks + this.attacker.effectBonus) * this.attacker.effectFactor));
  }
  ...
  {
    this.AddStatusEffect(new CardData.StatusEffectStacks()
    {
