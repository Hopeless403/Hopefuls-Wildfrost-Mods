...
public class Entity : MonoBehaviourCacheTransform, IPoolable
{
  ...
  private CardContainer[] _preContainers;
  ...
  private int traitUpdateRunning;
  ...
  private void Awake() => this.random3 = PettyRandom.Vector3();
  ...
  private void Update()
  {
    ...
    if (!this.alive || !this.ReadyToDie())
      return;
    CoroutineManager.Start(this.Kill(this.forceKill == DeathType.None ? DeathType.Normal : this.forceKill));
    this.forceKill = DeathType.None;
  }

  public bool ReadyToDie()
  {
    if (this.forceKill != DeathType.None)
      return true;
  ...
  private void OnDisable() => Events.InvokeEntityDisabled(this);
