...
public class AddressableLoader : MonoBehaviour
{
  ...
  private static bool StartLoadGroup(string name, out AsyncOperationHandle<IList<DataFile>> handle)
  {
    if (AddressableLoader.IsGroupLoaded(name))
    {
  ...
  private static void StoreGroup<T>(string name, ICollection<T> data) where T : DataFile
  {
    if (data == null)
  ...
  private class Group
  {
    private readonly Dictionary<string, DataFile> lookup = new Dictionary<string, DataFile>();
    private readonly List<DataFile> list = new List<DataFile>();
    ...
    private static string GetName(DataFile asset)
    {
