...
public abstract class StatusEffectApplyX : StatusEffectData
{
  ...
  [SerializeField]
  private SelectScript<Entity> selectScript;
  public TargetConstraint[] applyConstraints;
  public bool applyEqualAmount;
  [ShowIf("applyEqualAmount")]
  [SerializeField]
  protected ScriptableAmount contextEqualAmount;
  [ShowIf("applyEqualAmount")]
  [SerializeField]
  protected float equalAmountBonusMult;
  [HideIf("applyEqualAmount")]
  [SerializeField]
  protected ScriptableAmount scriptableAmount;
  ...
  [SerializeField]
  private NoTargetType noTargetType;
  [SerializeField]
  private string[] noTargetTypeArgs;
  private ActionEffectApply action;

  protected override bool CanTrigger()
  {
    return base.CanTrigger() && this.dealDamage || (bool) (UnityEngine.Object) this.effectToApply;
  }

  protected virtual int GetAmount(Entity entity, bool equalAmount = false, int equalTo = 0)
  {
  ...
  protected virtual bool TargetSilenced() => this.target.silenced;

  protected bool AppliesTo(StatusEffectApplyX.ApplyToFlags applyTo)
  {
    return (this.applyToFlags & applyTo) != 0;
  }

  protected IEnumerator Run(List<Entity> targets, int amount = 0)
  {
  ...
  private bool CheckMultipleEffectsForUnboostable()
  {
  ...
  public IEnumerator Sequence(List<Entity> targets, int amount)
  {
    ...
    if ((bool) (UnityEngine.Object) statusEffectApplyX.target && (statusEffectApplyX.target.alive || !statusEffectApplyX.targetMustBeAlive))
    {
      ...
      if ((bool) (UnityEngine.Object) statusEffectApplyX.selectScript)
        targets = statusEffectApplyX.selectScript.Run(targets);
      ...
      else
      {
        for (int index = targets.Count - 1; index >= 0; --index)
        {
          if (!(bool) (UnityEngine.Object) targets[index] || !statusEffectApplyX.CheckConstraints(targets[index]))
            targets.RemoveAt(index);
        }
        if (targets.Count > 0)
        {
  ...
  private IEnumerator SequenceSingle(Entity t, int amount)
  {
    StatusEffectApplyX statusEffectApplyX = this;
...
label_18:
  ...
  private void RemoveIneligible(IList<Entity> list)
  {
  ...
  private bool CanAffect(Entity entity)
  {
    return !this.dealDamage ? this.effectToApply.CanPlayOn(entity) : entity.canBeHit;
  }

  public bool CheckConstraints(Entity entity)
  {
    return this.CanAffect(entity) && ((IEnumerable<TargetConstraint>) this.applyConstraints).All<TargetConstraint>((Func<TargetConstraint, bool>) (c => c.Check(entity)));
  }
