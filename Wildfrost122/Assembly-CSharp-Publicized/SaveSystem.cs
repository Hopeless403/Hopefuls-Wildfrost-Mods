...
public class SaveSystem : GameSystem
{
  ...
  {
    get => (UnityEngine.Object) SaveSystem.instance != (UnityEngine.Object) null && SaveSystem.instance.enabled;
  }
  ...
  {
    return SaveSystem.LoadProgressData<T>(key, default (T));
  }

  public static T LoadCampaignData<T>(GameMode gameMode, string key) where T : class
  {
    return SaveSystem.LoadCampaignData<T>(gameMode, key, default (T));
  }

  public static T LoadStatsData<T>(string key) where T : class
  {
    return SaveSystem.LoadStatsData<T>(key, default (T));
  }

  public static T LoadHistoryData<T>(string key) where T : class
  {
    return SaveSystem.LoadHistoryData<T>(key, default (T));
  }

  public static T LoadProgressData<T>(string key, T defaultValue)
  {
    return !SaveSystem.Enabled ? defaultValue : SaveSystem.progressSaver.LoadValue<T>(key, SaveSystem.folderName, defaultValue);
  }

  public static T LoadCampaignData<T>(GameMode gameMode, string key, T defaultValue)
  {
    return !SaveSystem.Enabled ? defaultValue : SaveSystem.campaignSaver.LoadValue<T>(key, SaveSystem.folderName, defaultValue, gameMode.saveFileName);
  }

  public static T LoadStatsData<T>(string key, T defaultValue)
  {
    return !SaveSystem.Enabled ? defaultValue : SaveSystem.statsSaver.LoadValue<T>(key, SaveSystem.folderName, defaultValue);
  }

  public static T LoadHistoryData<T>(string key, T defaultValue)
  {
    return !SaveSystem.Enabled ? defaultValue : SaveSystem.historySaver.LoadValue<T>(key, SaveSystem.folderName, defaultValue);
  }

  public static bool ProgressExists()
  {
    return SaveSystem.Enabled && SaveSystem.progressSaver.FileExists(SaveSystem.folderName);
  }

  public static bool CampaignExists(GameMode gameMode)
  {
    return SaveSystem.Enabled && SaveSystem.campaignSaver.FileExists(SaveSystem.folderName, gameMode.saveFileName);
  }

  public static bool StatsExists()
  {
    return SaveSystem.Enabled && SaveSystem.statsSaver.FileExists(SaveSystem.folderName);
  }

  public static bool HistoryExists()
  {
    return SaveSystem.Enabled && SaveSystem.historySaver.FileExists(SaveSystem.folderName);
  }

  public static bool ProgressDataExists(string key)
  {
    return SaveSystem.Enabled && SaveSystem.progressSaver.KeyExists(key, SaveSystem.folderName);
  }

  public static bool CampaignDataExists(GameMode gameMode, string key)
  {
    return SaveSystem.Enabled && SaveSystem.campaignSaver.KeyExists(key, SaveSystem.folderName, gameMode.saveFileName);
  }

  public static bool StatsDataExists(string key)
  {
    return SaveSystem.Enabled && SaveSystem.statsSaver.KeyExists(key, SaveSystem.folderName);
  }

  public static bool HistoryDataExists(string key)
  {
    return SaveSystem.Enabled && SaveSystem.historySaver.KeyExists(key, SaveSystem.folderName);
  }
