...
public class TutorialBattleSystem1 : TutorialBattleSystem
{
  ...
  public class PhasePlaceCompanion : TutorialParentSystem.Phase
  {
    ...
    {
      return action is ActionMove actionMove && actionMove.entity.data.name == this.target.name && actionMove.toContainers.Length == 1 && Battle.IsOnBoard(actionMove.toContainers[0].Group);
    }

    [CompilerGenerated]
    public string \u003COnEnable\u003Eb__4_0()
    {
      return string.Format(MonoBehaviourSingleton<StringReference>.instance.tutorialBattle1_2.GetLocalizedString(), (object) this.target.title);
    }
  }
  ...
  public class PhaseUseItem : TutorialParentSystem.Phase
  {
    ...
    public bool CorrectAction(PlayAction action)
    {
      return action is ActionTriggerAgainst actionTriggerAgainst && actionTriggerAgainst.entity.data.name == "Sword" && actionTriggerAgainst.target.data.name == "Pengoon";
    }
  }
