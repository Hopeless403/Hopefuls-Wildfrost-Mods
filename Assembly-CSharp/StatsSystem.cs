...
public class StatsSystem : GameSystem
{
  ...
  [SerializeField]
  protected CampaignStats stats;
  private int goldThisBattle;
  private int sacrificedThisBattle;
  private bool kingMokoExists;
  private bool campaignEnded;
  ...
  private CampaignStats Stats => this.stats;
  ...
  private void CampaignEnd(Campaign.Result result, CampaignStats stats, PlayerData playerData) => this.campaignEnded = true;

  private void CampaignSaved()
  {
  ...
  private void PostEntityHit(Hit hit)
  {
  ...
  private void EntityOffered(Entity entity) => this.Stats.Add("cardsOffered", entity.data.name, 1);
  ...
  private void EntityDiscarded(Entity entity)
  {
    if (!((UnityEngine.Object) entity.owner == (UnityEngine.Object) References.Player))
      return;
  ...
  private void EntityTriggered(ref Trigger trigger)
  {
  ...
  private void CardInjured(CardData cardData) => this.Stats.Add("friendliesInjured", cardData.name, 1);
  ...
  private void SpendGold(int amount) => this.Stats.Add("goldSpent", amount);

  private void ShopItemPurchase(ShopItem item)
  {
  ...
  private void KillCombo(int combo) => this.Stats.Max("highestKillCombo", combo);

  private void RedrawBellHit(RedrawBellSystem redrawBellSystem) => this.Stats.Add("redrawBellHits", 1);

  private void BattleTurnStart(int turnCount) => this.Stats.Add("turnsTaken", 1);
  ...
  private void UpgradeAssigned(Entity entity, CardUpgradeData upgradeData)
  {
  ...
  private static string GetBattleName()
  {
    object obj;
