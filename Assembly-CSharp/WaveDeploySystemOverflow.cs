...
public class WaveDeploySystemOverflow : GameSystem, ISaveable<BattleWaveData>
{
  ...
  [SerializeField]
  private Color glowImminentColor = Color.red;
  [Header("Deploy Early")]
  [SerializeField]
  private int deployEarlyReward;
  [SerializeField]
  private int deployEarlyRewardPerTurn = 5;
  [SerializeField]
  private bool autoEarlyDeploy = true;
  [SerializeField]
  private bool canCallEarly;
  [Header("Popup")]
  [SerializeField]
  private LocalizedString popupDesc;
  [SerializeField]
  private LocalizedString popupHitDesc;
  [SerializeField]
  private LocalizedString popupOverflowDesc;
  [SerializeField]
  private LocalizedString popupRewardDesc;
  [Header("Settings")]
  [SerializeField]
  private KeywordData popup;
  [SerializeField]
  ...
  private float fadeAdd = 5f;
  private bool popped;
  private List<BattleWaveManager.Wave> waves;
  private List<Entity> deployedThisTurn;
  private List<ulong> deployed;
  private int overflowWaveIndex = -1;
  ...
  private void AssignCardController(CardController controller) => this.GetComponentInChildren<ToggleBasedOnCardController>(true)?.AssignCardController(controller);
  ...
  private static bool CheckEarlyDeploy() => Battle.GetCardsOnBoard(References.Battle.enemy).Count <= 0;
  ...
  private void Overflow(IEnumerable<Entity> entities)
  {
  ...
  private void CreateOverflowWave()
  {
  ...
  private bool TryToAddToOverflow(Entity entity)
  {
  ...
  private IEnumerator EarlyDeploy()
  {
    InputSystem.Disable();
  ...
  private static int CountEmptySpacesOnBoard() => References.Battle.GetRows(References.Battle.enemy).Sum<CardContainer>((Func<CardContainer, int>) (a => !a.canBePlacedOn ? 0 : a.max - a.Count));
