...
public class ScrollToNavigation : MonoBehaviour
{
  ...
  private void ScrollScrollerVertical(Transform target) => this.scroller.ScrollTo(this.scroller.targetPos.WithY(this.scroller.transform.position.y - target.position.y));

  private void ScrollScrollerHorizontal(Transform target) => this.scroller.ScrollTo(this.scroller.targetPos.WithX(this.scroller.transform.position.x - target.position.x));
  ...
  private void ScrollScrollRectVertical(Transform target) => this.scrollRect.ScrollTo(this.scrollRect.content.anchoredPosition.WithY((float) -((double) target.position.y - (double) this.scrollRect.content.position.y + (double) this.scrollRect.viewport.rect.size.y * 0.5)));

  private void ScrollScrollRectHorizontal(Transform target) => this.scrollRect.ScrollTo(this.scrollRect.content.anchoredPosition.WithX((float) -((double) target.position.x - (double) this.scrollRect.content.position.x + (double) this.scrollRect.viewport.rect.size.x * 0.5)));
}
