...
namespace Rewired.Integration.UnityUI
{
  public abstract class RewiredPointerInputModule : BaseInputModule
  {
    ...
    private RewiredPointerInputModule.UnityInputSource defaultInputSource => this.__m_DefaultInputSource == null ? (this.__m_DefaultInputSource = new RewiredPointerInputModule.UnityInputSource()) : this.__m_DefaultInputSource;

    private IMouseInputSource defaultMouseInputSource => (IMouseInputSource) this.defaultInputSource;

    protected ITouchInputSource defaultTouchInputSource => (ITouchInputSource) this.defaultInputSource;

    protected bool IsDefaultMouse(IMouseInputSource mouse) => this.defaultMouseInputSource == mouse;
    ...
    public ITouchInputSource GetTouchInputSource(int playerId, int sourceIndex) => !UnityTools.IsNullOrDestroyed<ITouchInputSource>(this.m_UserDefaultTouchInputSource) ? this.m_UserDefaultTouchInputSource : this.defaultTouchInputSource;
    ...
    public void AddTouchInputSource(ITouchInputSource source) => this.m_UserDefaultTouchInputSource = !UnityTools.IsNullOrDestroyed<ITouchInputSource>(source) ? source : throw new ArgumentNullException(nameof (source));

    public int GetTouchInputSourceCount(int playerId) => !this.IsDefaultPlayer(playerId) ? 0 : 1;

    protected void ClearMouseInputSources() => this.m_MouseInputSourcesList.Clear();
