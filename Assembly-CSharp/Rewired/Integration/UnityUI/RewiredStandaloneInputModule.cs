...
namespace Rewired.Integration.UnityUI
{
  ...
  public sealed class RewiredStandaloneInputModule : RewiredPointerInputModule
  {
    ...
    private bool GetButton(Player player, int actionId) => actionId >= 0 && player.GetButton(actionId);

    private bool GetButtonDown(Player player, int actionId) => actionId >= 0 && player.GetButtonDown(actionId);

    private bool GetNegativeButton(Player player, int actionId) => actionId >= 0 && player.GetNegativeButton(actionId);

    private bool GetNegativeButtonDown(Player player, int actionId) => actionId >= 0 && player.GetNegativeButtonDown(actionId);

    private float GetAxis(Player player, int actionId) => actionId < 0 ? 0.0f : player.GetAxis(actionId);
