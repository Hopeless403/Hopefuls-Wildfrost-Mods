...
namespace Deadpan.Enums.Engine.Components.Modding
{
  public class CardUpgradeDataBuilder : DataFileBuilder<CardUpgradeData, CardUpgradeDataBuilder>
  {
    ...
    public override CardUpgradeDataBuilder Create(string name)
    {
      if (this.Mod != null)
        name = Extensions.PrefixGUID(name, this.Mod);
      if (this._data != null)
        UnityEngine.Object.Destroy((UnityEngine.Object) this._data);
      this._data = ScriptableObject.CreateInstance<CardUpgradeData>();
    ...
    private void UnlockSequenceOnOnStart<T>(BuildingSequenceWithUnlocks<T> obj) where T : BuildingSequenceWithUnlocks<T> => obj.AddChallengeStone(this.UnlockChallenge);

    public CardUpgradeDataBuilder WithImage(string img)
    {
      this._data.image = Extensions.GetCardSprite(Path.Combine(this.Mod.ImagePath(img)));
      return this;
    }
