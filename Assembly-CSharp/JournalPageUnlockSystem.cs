...
public class JournalPageUnlockSystem : GameSystem
{
  ...
  private void OnEnable() => Events.OnCampaignLoadPreset += new Events.UnityActionRef<string[]>(this.InsertJournalPages);

  private void OnDisable() => Events.OnCampaignLoadPreset -= new Events.UnityActionRef<string[]>(this.InsertJournalPages);
  ...
  private static bool IsLegal(JournalPageData page) => page.legalGameModes.Contains<GameMode>(Campaign.Data.GameMode) && page.legalTribes.Contains<ClassData>(References.PlayerData.classData) && JournalPageUnlockSystem.HasRequiredStormPoints(page) && JournalPageUnlockSystem.HasRequiredModifiers(page);

  private static bool HasRequiredStormPoints(JournalPageData page) => page.requiresStormPoints <= 0 || StormBellManager.GetCurrentStormPoints(StormBellManager.GetActiveStormBells()) >= page.requiresStormPoints;
