...
public class CardController : MonoBehaviour
{
  ...
  protected Vector3 draggingPositionPre;
  protected int draggingLayerPre;
  protected Entity pressEntity;
  private bool press;

  public static CardController Find(GameObject gameObject) => gameObject.GetComponentInParent<CardController>();

  protected virtual bool AllowDynamicSelectRelease => InputSystem.AllowDynamicSelectRelease;

  private Vector3 CardHoverScale => new Vector3(this.cardHoverScale * 1f, this.cardHoverScale * 1f, 1f);
  ...
  private void OnEnable() => Events.InvokeCardControllerEnabled(this);
  ...
  private void OnDisable() => Events.InvokeCardControllerDisabled(this);
  ...
  protected bool TryDrag(Entity entity)
  {
  ...
  private void Drag(Entity entity)
  {
  ...
  protected Vector3 GetDragPosition() => (Cursor3d.Position - (this.dragging.offset.position - this.dragging.transform.position)).WithZ(this.dragZ);

  protected virtual void DragUpdatePosition() => this.dragging.transform.position = Delta.Lerp(this.dragging.transform.position, this.GetDragPosition(), this.dragLerp, Time.deltaTime);
